...
import { randomColor } from "../../../config/colors";
...
const GameMatch = ({
  ...
  const pairs = React.useMemo(() => {
    if (!(vocabulary && topic?.vocabulary)) return null;

    const topicVocabulary = Object.values(topic?.vocabulary ?? {});

    const result = shuffle(
      Object.entries(vocabulary).filter((entry) =>
        topicVocabulary.includes(entry[0])
      )
    )
      .map((entry) => ({
        text: entry[0],
  ...
  const checkAnswer = () => {
    return pairs?.some(
      (p) =>
        p.text === textOptions[selection.textInd] &&
        p.image === imageOptions[selection.imageInd]
    );
  };
