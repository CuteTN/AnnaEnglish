...
const ResultWordButton = ({ word, backgroundColor, onPress }) => {
  const handlePress = () => {
    onPress?.();
  };
...
const GameSort = ({ data, onComplete, onStepChange }) => {
  ...
  const numcolResultWordButton = (questions[currentStep]?.answer.length < 8) ? 7 : Math.floor((questions[currentStep]?.answer.length + 1) / 2);
  ...
  const checkAnswer = () => {
    ...
    const userAnswer = selectedIndices.map(index => words[index] ?? " ").join("");
