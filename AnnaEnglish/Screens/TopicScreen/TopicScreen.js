...
export default TopicScreen = ({ route }) => {
  ...
  useEffect(() => {
    ...
    if (!userProgressOnTopic)
      return;

    const completedGames = Object.entries(userProgressOnTopic.completedGames ?? {});
  ...
  const handleGameComplete = (gameId) => {
    ...
    if (!completedGamesInTopicToday.current.includes(gameId)) {
      ...
      if (games.length === completedGamesInTopicToday.current.length && !flagFirstCompleted) {
